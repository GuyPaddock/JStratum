 ...
 * the message, along with an optional <em>subject key</em> that the recipient can use to refer to
 * the context at a later time in the Stratum conversation.</p>
 *
 ...
 * result in response to the {@code mining.subscribe} method call can look like the following:</p>
 ...
 * For example, in the Electrum implementation of Stratum, a valid result in response to the
 * {@code blockchain.address.get_history} method call can look like the following:</p>
...
implements Result {
  /**
   * An optional subject that can provide the receiving party with additional context about this
   * result.
   */
  private String subject;

  /**
   * An optional unique identifier for the specific subject of this result, which the recipient can
   * use to refer to the context at a later time in the Stratum conversation.</p>
   */
   ...
   *
   * @param resultData
   *   Data, if any, that should be contained in the result (after the subject tuple).
   */
  public ArrayResult(final Object... resultData) {
   ...
   *
   * @param subject
   *   An optional subject that can provide the receiving party with additional context about the
   *   result.
   * @param subjectKey
   *   An optional unique identifier for the specific subject of the result, which the recipient can
   *   use to refer to the context at a later time in the Stratum conversation. This must be
   *   {@code null} if {@code subject} is {@code null}.
   * @param resultData
   *   Data, if any, that should be contained in the result (after the subject tuple).
   *
   * @throws IllegalArgumentException
   *   If a non-{@code null} value is provided for {@code subjectKey} when {@code subject} is
   *   {@code null}.
   */
  public ArrayResult(final String subject, final String subjectKey, final Object... resultData)
  throws IllegalArgumentException {
  ...
  /**
   * Constructor for {@link ArrayResult} that initializes a new instance from information in the
   * included JSON array.
   *
   ...
   *   understood.
   */
  public ArrayResult(final JSONArray jsonResultData)
  throws MalformedStratumMessageException {
    this.parseResult(jsonResultData);
  }

  /**
   * Gets the optional subject that can provide the receiving party with additional context about
   * this result.
   *
   * @return Either the subject of this message; or {@code null} if no subject was specified.
   */
  ...
  /**
   * Gets the optional unique identifier for the specific subject of this result, which the
   * recipient can use to refer to the context at a later time in the Stratum conversation.
   *
   * @return Either the subject of this message; or {@code null} if no subject was specified.
   */
  ...
  public JSONArray toJson() {
    ...
    final Object    subjectKey = this.getSubjectKey();

    if (subject != null) {
      final JSONArray subjectTuple = new JSONArray();
   ...
   *
   * @throws IllegalArgumentException
   *   If a {@code null} value is provided for {@code subject} when the subject key of this result
   *   is not {@code null}.
   */
  ...
  throws IllegalArgumentException {
    if ((subject == null) && (this.getSubjectKey() != null)) {
      throw new IllegalArgumentException(
        "Subject cannot be set to null if subject key is not null."
      );
    }
   ...
   * result, which the recipient can use to refer to the context at a later time in the Stratum
   * conversation.</p>
   *
   ...
   *
   * @throws IllegalArgumentException
   *   If a non-{@code null} value is provided for {@code subjectKey} when the subject of this
   *   result is {@code null}.
   */
  protected void setSubjectKey(final String subjectKey)
  throws IllegalArgumentException {
  ...
  /**
   * Sets the list of the data contained in this result (after the subject tuple).
   *
   * @param resultData
   *   The new list of result data.
   */
  protected void setResultData(final List<Object> resultData) {
    this.resultData = resultData;
  }

  /**
   * Parses the provided JSON array as a Stratum array result and then populates this result
   * accordingly with its contents.
   *
   * @param jsonResultData
   *   The array of result data to parse.
   *
   * @throws MalformedStratumMessageException
   *   If the provided JSON array is not a properly-formed Stratum result or cannot be understood.
   */
  protected void parseResult(final JSONArray jsonResultData)
  throws MalformedStratumMessageException {
    final List<Object>  resultData    = new ArrayList<>();
    int                 startingIndex = 0;
    ...
    for (int arrayIndex = startingIndex; arrayIndex < jsonResultData.length(); ++arrayIndex) {
      ...
      try {
        ...
        throw new RuntimeException(
          "Unexpected exception while retrieving JSON element: " + ex.getMessage(),
          ex
        );
      }
  ...
  /**
   * Determines whether or not the provided JSON result data array contains a subject tuple, parses
   * it if it is present, and then populates the subject information of this result accordingly with
   * its contents.
   *
   * @param jsonResultData
   *   The array of result data to parse.
   *
   * @return {@code true} if a subject tuple was present and parsed; {@code false}, otherwise.
   *
   * @throws MalformedStratumMessageException
   *   If the provided JSON array does not contain a properly-formed Stratum result subject tuple,
   *   or cannot be understood.
   */
  protected boolean parseOptionalSubjectTuple(final JSONArray jsonResultData)
  throws MalformedStratumMessageException {
