 ...
 * it to gracefully progress through conversations of varying complexity with a Stratum client.</p>
 *
 * <p>This implementation also automatically closes and expires connections
 * that sit idle for longer than {@link StratumTcpServerConnection#MAX_IDLE_TIME_MSECS}. The {@link
 * #onConnectionTimeout(StratumTcpServerConnection)} method is invoked each time that a connection
 * expires.</p>
 *
...
public abstract class StratumTcpServer {
  ...
  /**
   * Resets the timeout counter for the provided connection, preventing the connection from expiring
   * until another {@link StratumTcpServerConnection#MAX_IDLE_TIME_MSECS} have passed without this
   ...
   *
   * @param connectionId
   *   The unique identifier for the connection for which the timer is being reset.
   *
   * @throws IllegalArgumentException
   *   If the connection is not known to this server.
   */
  ...
  throws IllegalArgumentException {
    ...
    if (connection == null) {
      throw new IllegalArgumentException(
        "No such connection is known to this server: " + connectionId
      );
    }
  }
   ...
   * connection map, including its rules for expiring stale connections and the limit on how many
   * connections can be active at a single time.</p>
   *
